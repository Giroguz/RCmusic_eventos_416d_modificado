...
import { getDemoDays, getStoredDjSession, getSubscriptionPlanPrices, getSubscriptionQr, getSubscriptionYapeNumber, requestAdminCodeNotification, sendEmailVerificationLink, signInDj, signOutDj, startDjTrial, submitSubscriptionProof, supabase, supabaseEnabled } from '../lib/supabase'
...
export default function DjLogin({ onLogin, onBack, developerMode = false }) {
  ...
  async function generateTrial() {
    ...
    sessionStorage.removeItem('rc_pending_trial_v1'); setTrialCode(result.generated_code); setTrialStep('done'); setEmail(trialEmail); setCode(result.generated_code); setTrialError('Correo verificado. Tu usuario ya aparece en el Centro de gestión.')
  }
  ...
  useEffect(() => {
    ...
    const finishRecovery = async (session) => {
      try {
        ...
        if (!pending || !session?.user?.email || session.user.email.toLowerCase() !== pending.email || !session.user.email_confirmed_at) return
        if (!active) return
        sessionStorage.removeItem('rc_pending_recovery_v1'); setRecoveryEmail(pending.email); setRecoveryVerified(true); setRecoveryStep('new-password'); setRecoveryMessage('Correo verificado. Ahora define tu nueva clave de desarrollador.')
  ...
  async function requestAdminCodeByEmail() {
    ...
    try {
      await requestAdminCodeNotification(requester)
      setError('Solicitud registrada. El desarrollador recibirá una notificación por WhatsApp y te responderá por el canal correspondiente.')
    } catch {
  ...
  async function requestRecovery(e) {
    ...
    try {
      ...
      sessionStorage.setItem('rc_pending_recovery_v1', JSON.stringify({ email: recoveryEmail.trim().toLowerCase() }))
  ...
  async function submit(e) {
    ...
    try {
      ...
      if (access?.planExpired) {
        setPlanExpired(true); setExpiredSession(access); setError(t('planExpired')); return
      }
      onLogin(access)
    } catch (err) {

...
export default function DjApp({ onExit, session = getStoredDjSession() }) {
  ...
  async function saveProfile(e) {
    ...
    try {
      ...
      if (supabaseEnabled && (Boolean(profileForm.tipsRequired) !== Boolean(activeEvent.tipsRequired) || String(profileForm.tipCurrency || 'PEN') !== String(activeEvent.tipCurrency || 'PEN') || Boolean(profileForm.fixedTipAmount) !== (Number(activeEvent.tipAmount || 0) > 0) || Number(profileForm.tipAmount || 0) !== Number(activeEvent.tipAmount || 0))) await updateDjEventTipSettings(activeEvent.id, profileForm.tipsRequired, profileForm.tipCurrency, profileForm.tipAmount, session?.token)
